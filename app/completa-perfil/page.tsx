    ...
    <div className="w-full flex items-center justify-center px-4 pt-16 sm:px-6 sm:pt-12 lg:px-[40px] lg:pt-16 pb-4 sm:pb-6 lg:pb-[40px]">
      <div className="w-full max-w-6xl relative flex flex-col lg:flex-row items-center gap-8 sm:gap-10 lg:gap-[54px]">
        <div className="flex-1 flex flex-col items-start justify-center gap-4 w-full lg:w-auto">
          <b className="self-stretch relative tracking-[-1.5px] leading-[50px] lg:leading-[60px] text-[32px] sm:text-[36px] lg:text-[42px] font-bold">Completa tu perfil</b>
          ...
          <div className="self-stretch flex flex-col gap-4">
            {steps.map((step, index) => (
              <div
                ...
                className={`self-stretch rounded-[18px] flex items-center py-4 px-8 cursor-pointer transition-all ${
                  selectedStep === index
          ...
          {/* CTA Button */}
          <Link href="/conecta-cuentas" className="w-full sm:w-auto mt-6">
        ...
        <div className="h-[500px] sm:h-[550px] lg:h-[600px] w-full max-w-[300px] lg:w-[277.3px] relative rounded-[40px] bg-[#EDEEEC] border-black border-solid border-[8px] sm:border-[10px] lg:border-[11px] box-border overflow-hidden shrink-0 flex items-center justify-center">
          <div className="relative w-full h-full">
            {steps.map((step, index) => (
              <div
                key={step.id}
                className={`absolute inset-0 w-full h-full transition-opacity duration-300 ${
                  selectedStep === index && !isTransitioning
                    ? "opacity-100 z-10"
                    : "opacity-0 z-0"
                }`}
              >
